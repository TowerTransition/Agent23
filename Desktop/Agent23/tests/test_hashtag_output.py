...
- Exactly 4 hashtags on every post (regardless of platform)
- Hashtag patterns: #DomainSpecific #ConceptName #ClarityMatters #RealWorldAI
"""
...
class TestHashtagOutput(unittest.TestCase):
    ...
    def test_text_generator_ensures_hashtags(self):
        """Test that text_generator._ensure_hashtags generates exactly 4 hashtags (training aligned)."""
        ...
        # Training data shows exactly 4 hashtags regardless of platform
        test_cases = [
            ({"lens_plan": {"domain": "Foreclosures"}}, "twitter"),
            ({"lens_plan": {"domain": "Trading Futures"}}, "facebook"),
            ({"lens_plan": {"domain": "Assisted Living"}}, "instagram"),
            ({"lens_plan": {"domain": "General"}}, "linkedin"),
        ]

        EXPECTED_HASHTAG_COUNT = 4  # Training data always has exactly 4 hashtags

        for context, platform in test_cases:
            hashtags = gen._ensure_hashtags(context, platform)
            # Check exact count matches training data
            self.assertEqual(len(hashtags), EXPECTED_HASHTAG_COUNT,
                f"Should generate exactly {EXPECTED_HASHTAG_COUNT} hashtags for {platform}, got {len(hashtags)}")
