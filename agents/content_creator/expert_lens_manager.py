...
class ExpertLensManager:
    ...
    - Tracking which domains/content have been covered to avoid repetition
    ...
    def pick_plan(
        ...
        trend_candidates: Optional[List[Dict[str, Any]]] = None  # Backward compatibility
    ) -> Dict[str, Any]:
        """
        Select an expert lens, pick content, and create a content plan.
        
        This method:
        1. Rotates to the next lens in the fixed cycle
        2. Picks the best content candidate, avoiding recently covered domains
        3. Rotates brand phrases to avoid repetition
        4. Creates a plan that guides content generation
        
        Args:
            candidates: List of content dictionaries with keys:
                - "title" or "context": Content title/description
                - "domain": Domain/category (e.g., "Foreclosures", "Trading Futures", "Assisted Living")
                - "score": Optional relevance score (higher is better)
                - "description": Optional detailed description
                - "hashtags": Optional list of hashtags
                - "lens_plan": Optional lens plan with additional context
            platform: Optional platform name (for platform-specific planning)
            trend_candidates: DEPRECATED - use 'candidates' instead. Kept for backward compatibility.
        ...
        Returns:
            Dictionary containing:
                ...
                - "domain": Domain of selected content
        ...
        """
        # Backward compatibility: use trend_candidates if candidates not provided
        if candidates is None:
            candidates = trend_candidates or []
        """
        Select an expert lens, pick content, and create a content plan.
        ...
        # 2) Pick best content candidate, avoid repeating domain if possible
        ...
        # Try to pick a candidate from a domain not recently covered
        picked = None
        ...
        # If all domains were recently covered, use the highest-scoring candidate
        if not picked:
            picked = sorted_candidates[0] if sorted_candidates else {}
        
        # Extract content info (from title, context, or lens_plan)
        content_title = picked.get("title") or picked.get("context") or ""
        content_description = picked.get("description") or ""
        
        # Check lens_plan for additional context
        lens_plan = picked.get("lens_plan") or {}
        if not content_title:
            content_title = lens_plan.get("title") or lens_plan.get("context") or "AI in real-world workflows"
        if not content_description:
            content_description = lens_plan.get("description") or ""
        
        domain = picked.get("domain") or picked.get("category", "General")
        hashtags = picked.get("hashtags", [])
        
        self.logger.info(f"Selected content: {content_title} (domain: {domain})")
    ...
    def _get_lens_instructions(self, lens: str) -> tuple:
        ...
        instructions = {
            "What Everyone Gets Wrong": (
                "Map the common misconception or misunderstanding about this topic. "
            ...
            "The Real Constraint": (
                "Map the actual workflow or process where this applies. "
                "Identify the real bottlenecks and limitations.",
                "Name the specific constraint that limits effectiveness. "
                "This is the actual barrier, not the perceived one."
            ),
            "How This Plays Out in Practice": (
                "Map the step-by-step workflow of how this is actually implemented. "
        ...
        return instructions.get(lens, (
            "Map the real-world workflow where this applies.",
    ...
    def get_domain_workflow(self, domain: str) -> Optional[Dict[str, Any]]:
        ...
        Args:
            domain: Domain name (e.g., "Foreclosures", "Trading Futures", "Assisted Living")
